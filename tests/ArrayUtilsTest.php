...
namespace Garden\Utils\Tests;

use ArrayAccess;
use Garden\Utils\ArrayUtils;
use Garden\Utils\Tests\Fixtures\DumbArray;
use Garden\Utils\Tests\Fixtures\IterableArray;
...
class ArrayUtilsTest extends TestCase
{
     ...
     */
    public function testPluck(array $arr, array $paths, array $expected)
    {
        $actual = ArrayUtils::pluck($arr, $paths);
        $this->assertEquals($expected, $actual);
    }
     ...
     * @return void
     */
    public function testArrKeyExistsNotArrayLike(): void
    {
        $result = ArrayUtils::arrayKeyExists("somekey", false);
     ...
     * @return iterable
     */
    public function providePluck(): iterable
    {
        yield "basic pluck" => [
            [
                "key1" => "val1",
        ...
        yield "nested properties" => [
            ...
            ["key1.nested2", "key2.nested1.nestedDeep"],
            [
                "key1.nested2" => "val1.2",
                "key2.nested1.nestedDeep" => "val2.1.1",
            ],
        ];
    }

    /**
     * Test for {@link ArrayUtils::remapProperties()}
     */
    public function testRemapProperties(): void
    {
        $initial = [
            "a" => 1,
            "b" => 2,
        ...
        $actual = ArrayUtils::remapProperties($initial, $mapping);
        $this->assertEquals($expected, $actual);

        $actual = ArrayUtils::remapProperties($initial, $mapping, true);
        $this->assertEquals($expectedRemoved, $actual);
    }

    /**
     * Tests for {@link ArrayUtils::trimKeyPrefix()}
     *
     * @return void
     */
    public function testTrimKeyPrefix(): void
    {
        $initial = [
            "testing1" => "val1",
            "testong2" => "val2",
        ...
        $actual = ArrayUtils::trimKeyPrefix($initial, "test");
        $this->assertEquals($expected, $actual);
    }

    /**
     * Tests for {@link ArrayUtils::dotNotationToArray()}
     *
     * @param array $dotNotationArr
     * @param array $nestedArr
     * @return void
     *
     * @dataProvider provideDotNotation
     */
    public function testDotNotationToArray(array $dotNotationArr, array $nestedArr): void
    {
        $actual = ArrayUtils::dotNotationToArray($dotNotationArr);
        $this->assertEquals($nestedArr, $actual);
    }

    /**
     * Tests for {@link ArrayUtils::arrayToDotNotation()}
     *
     * @param array $dotNotationArr
     * @param array $nestedArr
     * @return void
     ...
     * @dataProvider provideDotNotation
     */
    public function testArrayToDotNotation(array $dotNotationArr, array $nestedArr): void
    {
        $actual = ArrayUtils::arrayToDotNotation($nestedArr);
        $this->assertEquals($dotNotationArr, $actual);
    }

    public function provideDotNotation(): iterable
    {
        yield "basic" => [
            [
                "discussion.discussionID" => 5,
                "discussion.name" => "hello world",
                "discussion.body" => "hello body",
                "discussion.category.name" => "A category",
                "discussion.breadcrumbs" => ["crumb1", "crumb2"],
                "comment.commentID" => 5,
                "comment.discussionID" => 5,
                "comment.body" => "hello body",
            ],
            [
                "discussion" => [
                    "discussionID" => 5,
    ...
    /**
     * Tests for {@link ArrayUtils::jsonNormalizeArray()}
     */
    public function testJsonNormalizeArray(): void
    {
        $initial = [new DumbArray(["item1" => 1]), new DumbArray(["item2" => 2])];

        $expected = [["item1" => 1], ["item2" => 2]];

        $actual = ArrayUtils::jsonNormalizeArray($initial);
        $this->assertEquals($expected, $actual);
    }
}
