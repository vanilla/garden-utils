...
final class ArrayUtils
{
    ...
    public static function getByPath(string $path, $array, $default = null)
    {
        ...
        $search =
             ...
             * @return mixed|null
             */
            function ($array, array $keys) use ($default, &$search) {
    ...
    public static function setByPath(string $path, &$array, $value)
    {
        ...
        $search =
             ...
             * @return mixed
             */
            function ($array, array $keys) use ($value, &$search) {
                $currentKey = reset($keys);
    ...
    public static function walkRecursiveArray(&$array, callable $callback): void
    {
        $m =
             ...
             * @return void
             */
            function (&$array, array $path) use ($callback, &$m): void {
                if (is_iterable($array)) {
                    ...
                    foreach ($keys as $key) {
                        ...
                        if (isset($array[$key]) && self::isArray($array[$key])) {
     ...
     * and not each value.
     *
     * @param array|(iterable&ArrayAccess) $array The nested array to filter.
    ...
    public static function filterRecursiveArray($array, callable $filter, bool $includeRoot = false)
    {
        $m =
            /**
             * @param array|(iterable&ArrayAccess) $array
             * @param array $path
             * @return array
             */
            function (&$array, array $path) use ($filter, &$m): array {
                $r = [];
     ...
     * @param null|callable(array $arr1, array $arr2, string $key): mixed $numeric The merge function to use when numeric arrays are encountered.
    ...
    {
        if ($numeric === null) {
            $numeric = function (array $arr1, array $arr2): array {
                return array_values(array_unique(array_merge($arr1, $arr2)));
            };
        }
        /**
         * @var callable(array $arr1, array $arr2, string $key): mixed $numeric
         */
        ...
        $clean = function (array &$arr, array $arr1, array $arr2) use (&$clean, $numeric): void {
    ...
    /**
     * Make a filter function based on a simple where like filter array.
     *
    ...
    public static function filterCallback(array $where, bool $strict = false): callable
    {
        $callback =
             ...
             */
            function ($row) use ($where, $strict): bool {
                foreach ($where as $key => $value) {
                    if (!self::arrayKeyExists($key, $row)) {
                        return false;
     ...
     *
     * @param array|ArrayAccess $data The data to modify.
     * @param array $modifications The mapping of 'newName' => 'oldName'.
     * @param bool $remove If true, the values that were renamed will be removed.
     *
     * @return array|ArrayAccess
     */
    public static function remapProperties($data, array $modifications, bool $remove = false)
    {
        foreach ($modifications as $newName => $oldName) {
            $oldValue = self::getByPath($oldName, $data, null);
            self::setByPath($newName, $data, $oldValue);
        }

        if ($remove) {
            foreach ($modifications as $oldName) {
                self::unsetByPath($oldName, $data);
            }
        }
     ...
     *
     * @param mixed $arr
     * @param string[] $paths Dot-notation property paths.
     * @return array
     */
    public static function pluck($arr, array $paths): array
    {
        self::assertArray($arr, __METHOD__ . "() expects argument 1 to be an array or array-like object.");

        $result = [];
        foreach ($paths as $path) {
            $result[$path] = self::getByPath($path, $arr);
        }
     ...
     *
     * @param non-empty-string $glue The glue used to explode.
     ...
     */
    public static function dotNotationToArray(array $dotNotation)
    {
        $res = [];
